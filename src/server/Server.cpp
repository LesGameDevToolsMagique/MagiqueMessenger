...
int             Server::connection(const int domain, const int type, const std::string &protocol, const unsigned int max_listener)
{
...
int             Server::sendMessage(const void *message, const unsigned int msg_size, const int flags, const sockaddr *to)
{
    return this->getMessengerManager()->sendMessage(this->getConnectionManager()->getSocketFd(), message, msg_size, flags, to);
}

void            *Server::receiveMessage(const unsigned int read_size, const int flags, sockaddr *to, socklen_t *to_size)
{
